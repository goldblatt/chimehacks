...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        CITIES = [
                {'lat': 40.555, 'lng': -74.159},
                {'lat': 37.3230, 'lng': -122.0322},
                {'lat': -23.577734, 'lng': -46.625549},
                {'lat': -1.292021, 'lng': 36.806476},
                {'lat': 39.913992, 'lng': 116.389003},
                {'lat': 28.597453, 'lng': 77.221997},
                {'lat': 51.431817, 'lng': -0.070920}
            ]
...
"family having to read about this online. I kept reading. In the next paragraph, I read something that I will never forgive; I read that according to him, I liked it. I liked it. Again, I do not have words for these feelings."]
        ...
        for i in range(0, 7):
            ...
            story = Stories.objects.create(
                lng = CITIES[random_city]['lng'] + random.randint(0, 9) * 0.02,
                lat = CITIES[random_city]['lat'] + random.randint(0, 9) * 0.01,
